...
gulp.task('watch_blog_posts',function(){
    gulp.watch(paths.watcher_blog.styles_main_blog,['blog_post_styles']);
});
...
gulp.task('blog_post_styles',function(){
    // source to project 2 scss
    return gulp.src(paths.source.blog_src.blog_post.blog_post_style)
    .pipe(plumber({
    ...
    .pipe(gulp.dest(paths.blog_dest.blog_post.blog_post_styles))
    .pipe(cssmin()) // min css
    ...
    .pipe(gulp.dest(paths.blog_dest.blog_post.blog_post_styles))
    .pipe(notify({ message: 'blog_post_styles task finished' }))
