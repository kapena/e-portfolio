...
gulp.task('watch_blog',function(){
    gulp.watch(paths.watcher_blog.styles_main_blog,['blog_main_styles']);
});
...
gulp.task('blog_main_styles',function(){
    // source to project 2 scss
    return gulp.src(paths.source.blog_src.blog_home.blog_style)
    .pipe(plumber({
    ...
    .pipe(gulp.dest(paths.blog_dest.blog_home.blog_main_styles))
    .pipe(cssmin()) // min css
    ...
    .pipe(gulp.dest(paths.blog_dest.blog_home.blog_main_styles))
    .pipe(notify({ message: 'blog_main_styles task finished' }))
